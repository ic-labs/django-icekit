...
## In development
  ...
  * Add new "Can Republish" (`can_republish`) permission for all publishable
    items that permits users to publish items only if they have already been
    published, not if they are being published for the first time.
...
### Breaking changes:
  ...
  * The Workflow app available in earlier "develop" builds has been renamed.
    You will need to run an SQL statement manually to fix Django's migration
    history when upgrading a project that uses the 'icekit.workflow' app.
