...
class TestImageAPIViews(WebTest):
    ...
    def test_iiif_image_api_region(self):
        ...
        self.thumbnailer.get_thumbnail.assert_called_with({
            ...
            # NOTE: width is smallest edge
            'size': (self.image.width, self.image.width),
        })
        # Region: x,y,w,h
        self.app.get(
            reverse(
                'iiif_image_api',
                args=[self.image.pk, '20,30,50,90',
                      'max', '0', 'default', 'jpg']),
            user=self.superuser,
        )
        self.thumbnailer.get_thumbnail.assert_called_with({
        ...
        self.app.get(
            reverse(
                'iiif_image_api',
                args=[self.image.pk, '100,50,150,300',
                      'max', '0', 'default', 'jpg']),
            user=self.superuser,
        )
        self.thumbnailer.get_thumbnail.assert_called_with({
        ...
        self.app.get(
            reverse(
                'iiif_image_api',
                args=[self.image.pk, 'pct:10,50,75,50',
                      'max', '0', 'default', 'jpg']),
            user=self.superuser,
        )
        self.thumbnailer.get_thumbnail.assert_called_with({
    ...
    def test_iiif_image_api_size(self):
        ...
        self.app.get(
            reverse(
                'iiif_image_api',
                args=[self.image.pk, 'full', '100,150',
                      '0', 'default', 'jpg']),
            user=self.superuser,
        )
        self.thumbnailer.get_thumbnail.assert_called_with({
        ...
        self.app.get(
            reverse(
                'iiif_image_api',
                args=[self.image.pk, 'full', '!150,250',
                      '0', 'default', 'jpg']),
            user=self.superuser,
        )
        self.thumbnailer.get_thumbnail.assert_called_with({
        ...
        self.app.get(
            reverse(
                'iiif_image_api',
                args=[self.image.pk, 'full', '!300,240',
                      '0', 'default', 'jpg']),
            user=self.superuser,
        )
        self.thumbnailer.get_thumbnail.assert_called_with({
