...
from icekit.publishing.middleware import PublisherMiddleware, \
...
class TestPublishingModelAndQueryset(TestCase):
    ...
    def test_model_publishing_status_attributes(self):
        ...
        ``PublisherModelBase.is_dirty``
        """
        draft_instance = self.slide_show_1
        self.assertIsNone(draft_instance.publisher_linked)
        self.assertIsNone(draft_instance.publisher_published_at)
        ...
        published_instance = draft_instance.publisher_linked
        self.assertIsNotNone(published_instance.publisher_draft)
        self.assertIsNotNone(published_instance.publisher_published_at)
        ...
        draft_instance = published_instance.publisher_draft
        self.assertIsNotNone(draft_instance.publisher_linked)
        self.assertIsNotNone(draft_instance.publisher_published_at)
        self.assertTrue(draft_instance.is_draft)
        self.assertFalse(draft_instance.is_published)
        # Publishing timestamps correct?
        self.assertTrue(
            draft_instance.publisher_modified_at
            < draft_instance.publisher_linked.publisher_modified_at)
    ...
    def test_model_get_visible(self):
        ...
        with patch('icekit.publishing.models.get_draft_status') as p:
            ...
            self.assertEqual(
                self.slide_show_1.publisher_linked,
                self.slide_show_1.get_visible())
            self.assertEqual(
                self.slide_show_1.publisher_linked,
                self.slide_show_1.publisher_linked.get_visible())
        # In draft context...
        with patch('icekit.publishing.models.get_draft_status') as p:
            p.return_value = True
    ...
    def test_queryset_published(self):
        ...
        self.assertEqual(
            [self.slide_show_1.publisher_linked],  # Compare published copy
            list(SlideShow.objects.published()))
        ...
        with patch('icekit.publishing.managers.PublisherQuerySet.visible') \
                as p:
        ...
        self.assertEqual(
            set([self.slide_show_1.publisher_linked, self.slide_show_1]),
    ...
    def test_queryset_exchange_for_published(self):
        ...
        self.assertEqual(
            [p.pk for p in qs.filter(publisher_is_draft=False)],
    ...
    def test_model_get_draft(self):
        ...
        self.assertEqual(
            self.slide_show_1, self.slide_show_1.publisher_linked.get_draft())
    ...
    def test_model_get_published(self):
        ...
        self.assertEqual(
            self.slide_show_1.publisher_linked,
            self.slide_show_1.publisher_linked.get_published())
...
class TestPublishingMiddleware(TestCase):
    ...
    def test_middleware_method_is_admin_request(self):
        ...
        self.assertTrue(PublisherMiddleware.is_admin_request(request))
        # Not admin.
        request = self._request('/not-admin/', user=self.staff_1)
        self.assertFalse(PublisherMiddleware.is_admin_request(request))

    def test_middleware_method_is_staff_user(self):
        # Staff.
        request = self._request(user=self.staff_1)
        self.assertTrue(PublisherMiddleware.is_staff_user(request))
        # Reviewer.
        request = self._request(user=self.reviewer_user)
        self.assertFalse(PublisherMiddleware.is_staff_user(request))
        # Anonymous.
        self.assertFalse(
            PublisherMiddleware.is_staff_user(self._request()))
    ...
    def test_middleware_method_is_draft_request(self):
        ...
        self.assertTrue(PublisherMiddleware.is_draft_request(request))
        ...
        request = self._request(user=self.staff_1)
        self.assertFalse(PublisherMiddleware.is_draft_request(request))
    ...
    def test_middleware_method_is_draft(self):
        ...
        request = self._request(reverse('admin:index'), user=self.staff_1)
        self.assertTrue(PublisherMiddleware.is_draft(request))
        ...
        # ...or published.
        request = self._request(user=self.staff_1)
        self.assertFalse(PublisherMiddleware.is_draft(request))
...
class TestPublishingAdmin(BaseAdminTest):
    ...
    def test_publish_slideshow(self):
        ...
        self.assertIsNone(self.slide_show_1.publisher_linked)
        ...
        self.assertIsNotNone(self.slide_show_1.publisher_linked)
...
class TestFluentPublishing(TestCase):
    ...
    def test_queryset_published(self):
        ...
        self.slide_show_1.publisher_linked.publication_date = \
            timezone.now() + timedelta(seconds=1)
        self.slide_show_1.publisher_linked.save()
        self.assertEqual(
            set(), set(SlideShow.objects.published(with_exchange=False)))
        # Test publication date filtering: end date
        self.slide_show_1.publisher_linked.publication_date = None
        self.slide_show_1.publisher_linked.publication_end_date = \
            timezone.now() - timedelta(seconds=1)
        self.slide_show_1.publisher_linked.save()
        self.assertEqual(
            set(), set(SlideShow.objects.published(with_exchange=False)))
        # Test publication date filtering: clear start & end dates
        self.slide_show_1.publisher_linked.publication_end_date = None
        self.slide_show_1.publisher_linked.save()
        self.assertEqual(
            set([self.slide_show_1.publisher_linked, self.slide_show_1]),
            set(SlideShow.objects.published(with_exchange=False)))
