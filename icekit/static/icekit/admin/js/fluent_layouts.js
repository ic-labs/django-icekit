...
(function($)
{
  ...
  fluent_layouts.fetch_layout = function(layout_id)
  {
    ...
    if (!fluent_layouts.layout_url) {
    	alert("Internal CMS error: missing `fluent_layouts.layout_url` variable in the template!");
    }

    // Get layout info.
    $.ajax({
      url: fluent_layouts.layout_url + "placeholder_data/" + parseInt(layout_id) + "/?ct_id=" + ct_id,
